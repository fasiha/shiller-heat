...
function convert(data) {
  ...
  data.forEach(function(datum) {
    total += datum.amount;
    if (datum.amount < 0) { deposits += -datum.amount; }
    ...
    investments.push({amount: datum.amount, epochDays: epochDays});
  });
  if (start === end) { throw new Error('Transactions must not all be on the same day.'); }
  if (!negative) { throw new Error('Transactions must not all be nonnegative.'); }
  if (!nonnegative) { throw new Error('Transactions must not all be negative.'); }
  investments.forEach(function(investment) {
    // Number of years (including fraction) this item applies
    investment.years = (end - investment.epochDays) / DAYS_IN_YEAR;
  });
  return {total: total, deposits: deposits, days: end - start, investments: investments};
}

function xirr(transactions, options, guess) {
  ...
  var guess = guess || (data.total / data.deposits) / (data.days / DAYS_IN_YEAR);
